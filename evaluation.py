...
def computeAccuracy(predictions, gold_labels):
    ...
    for x in range(0, len(predictions)):
        if predictions[x] != gold_labels[x]:
            mistakes.append(x)
...
def computePrecisionRecall(predictions, gold_labels, relevant_class):
    ...
    for x in range(0, len(predictions)):
        if predictions[x] == gold_labels[x] and gold_labels[x] == relevant_class:
            true_positive.append(x)
        elif predictions[x] != gold_labels[x] and gold_labels[x] == relevant_class:
            false_negative.append(x)
        else:
            if predictions[x] != gold_labels[x] and predictions[x] != "NONE":
